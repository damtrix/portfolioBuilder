/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TechnicalSkillWhereUniqueInput } from "./TechnicalSkillWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { TechnicalSkillUpdateInput } from "./TechnicalSkillUpdateInput";

@ArgsType()
class UpdateTechnicalSkillArgs {
  @ApiProperty({
    required: true,
    type: () => TechnicalSkillWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => TechnicalSkillWhereUniqueInput)
  @Field(() => TechnicalSkillWhereUniqueInput, { nullable: false })
  where!: TechnicalSkillWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => TechnicalSkillUpdateInput,
  })
  @ValidateNested()
  @Type(() => TechnicalSkillUpdateInput)
  @Field(() => TechnicalSkillUpdateInput, { nullable: false })
  data!: TechnicalSkillUpdateInput;
}

export { UpdateTechnicalSkillArgs as UpdateTechnicalSkillArgs };
